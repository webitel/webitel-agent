...
class StreamRecorder implements RecorderI {
  ...
  Future<void> start({required String recordingId}) async {
    // [GUARD] Ensure any previous instance is fully disposed to avoid "captureSources: 2"
    ...
    try {
      ...
      // [WAIT] Wait for ICE gathering to complete.
      // Essential for getting candidates into the SDP before sending to server.
      int waitCount = 0;
  ...
  Future<void> _cleanupInternal() async {
    ...
    // 3. Force stop all media tracks (Crucial for UI indicators)
    for (final stream in streams) {
      for (final track in stream.getTracks()) {
        await track.stop();
      }
